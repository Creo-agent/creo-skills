...
1. [icon-font.css](icon-font.css) — Read with the Read tool and inline its **complete, unmodified contents** into `<style>` FIRST. Contains the `@font-face` with the base64-embedded WOFF2 icon font and the `.monday-icon` base class.
2. [design-system.css](design-system.css) — Read with the Read tool and inline its **complete, unmodified contents** into `<style>` AFTER icon-font.css. Never manually reconstruct or subset the CSS — paste the full file verbatim. This includes chart components (pie, donut, bar), table styles, stat blocks, and all animations.
...
Read `icon-font.css` and `design-system.css` with the Read tool and paste their **entire contents** into the `<style>` block, in that order (`icon-font.css` first). Do NOT manually recreate, summarize, or cherry-pick CSS rules. `icon-font.css` contains the base64-embedded WOFF2 font (~149KB) — inline all of it. `design-system.css` contains tokens, template classes, AND component styles (charts, tables, stat blocks, step cards, pie/donut charts, timelines, etc.) — inline all of it. If you skip sections, icons or components will silently fail to render.
...
```css
/* 1. icon-font.css inlined verbatim (@font-face + .monday-icon base) */
/* 2. design-system.css inlined verbatim (tokens, components, template classes) */
/* 3. Slide centering: position:absolute; top:50%; left:50%; transform:translate(-50%,-50%) */
/* 4. Per-slide overrides only if needed */
```
...
### Step 3.1: Icon Selection (Icon Font)
...
3. **Read [ICON_FONT_REFERENCE.md](ICON_FONT_REFERENCE.md)** — Find icons by category and semantic tags
4. **Select best match** — Choose icon that fits the slide content meaning
5. **Get HTML entity** — Copy the `&#xHHHH;` code from the reference
6. **Write the span** — `<span class="monday-icon ds-icon-md ds-icon-white">&#xHHHH;</span>`
...
- **NEVER use emojis** — always use icon font glyphs instead
- **Icons use white** — apply the `ds-icon-white` class on dark theme. Never recolor icons to multiple brand colors (e.g. green + red together)
...
Available size classes: `ds-icon-xs` (3vmin), `ds-icon-sm` (5vmin), `ds-icon-md` (7vmin, default), `ds-icon-lg` (10vmin), `ds-icon-xl` (15vmin).

Available color classes: `ds-icon-white` (white on dark, auto-inverts on light), `ds-icon-dark`, `ds-icon-brand` (purple), `ds-icon-muted` (60% opacity). No class = inherits `currentColor`.
...
- HTML: <span class="monday-icon ds-icon-md ds-icon-white">&#xf10b;</span>
```
...
<head>
  ...
  <style>
    /* INLINE ALL OF icon-font.css HERE (FIRST) */
    /* INLINE ALL OF design-system.css HERE (SECOND) */
...
**Critical CSS Rules:**
- Inline the **entire** [icon-font.css](icon-font.css) then [design-system.css](design-system.css) into the `<style>` block first
...
- **Icon font override:** When adding the Poppins font-family lockdown on `.slide-container *`, always add an override immediately after: `.slide-container .monday-icon { font-family: "MondayIcons" !important; font-style: normal !important; }` — without this, Poppins wins at equal specificity and icon glyphs break.
- **Icon coloring:** The icon font is monochrome — glyphs inherit CSS `color`. Use `.ds-icon-white`, `.ds-icon-dark`, `.ds-icon-brand` classes to set icon color. Without a color class, icons inherit `currentColor` from their parent.
...
4. If using icons: use the icon font — `<span class="monday-icon ds-icon-md ds-icon-white">&#xHHHH;</span>`. Look up entity codes in [ICON_FONT_REFERENCE.md](ICON_FONT_REFERENCE.md). Never use `<img src>` or inline SVGs for icons
...
| [icon-font.css](icon-font.css) | `@font-face` with base64-embedded WOFF2 icon font + `.monday-icon` base class. Inline FIRST into generated HTML. |
| [design-system.css](design-system.css) | CSS tokens, 19 template classes, and all reusable components. Inline AFTER icon-font.css into generated HTML. |
...
| [ICON_FONT_REFERENCE.md](ICON_FONT_REFERENCE.md) | **Phase 3.1** — 268 icon names, HTML entities, and semantic tags for icon selection |
...
- [ ] Icons use `<span class="monday-icon">&#xHHHH;</span>` pattern (no inline SVGs for icons)
