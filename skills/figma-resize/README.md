...
5. Applies Critical Rules 1–8 on every resize (geometric-mean scale, mask-group image rect fix, CROP imageTransform squash fix)
...
Four Figma-specific bugs caught in production — all fixed by Rules 1–8 in PROMPT.md:
...
- **Rule 5–7:** Post-resize pass using `absoluteBoundingBox` to fix image rects inside nested mask groups. The simple `imageRect.x = maskShape.x` shortcut breaks when wrapper groups have residual scale transforms — use the k-factor approach when the shortcut fails.
...
figma-resize/
  ...
  PROMPT.md   — Full execution workflow, Rules 1–8, per-size layout rules, QA step, quality checklist
  README.md   — This file
