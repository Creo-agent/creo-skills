...
5. Checks element placement (logo zone, CTA rules, visual container fidelity)
6. Measures safe zones and spacing with pixel thresholds
7. Scores the output on a 100-point weighted defect scale
8. Runs a 5-pillar visual inspection from the screenshots
9. Posts a structured VISION QA REPORT (wins, defects by pillar, exact fix instructions)
10. Saves the report to the deliverables folder
...
| Major (logo/CTA on edges, container styling lost, safe zone violation, ghost button) | −30 pts each |
| Minor (spacing below minimum, crowding, brand name line-break) | −10 pts each |
