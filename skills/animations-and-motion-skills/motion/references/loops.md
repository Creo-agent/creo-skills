...
- **Continuous:** endless motion with no start/end state (drifting, rotating). Just
  ...
  strips, card carousels) see the `motion` skill.
...
```
(This `/sc` correction applies **only** to elements measured *inside* `.stage-scaler`; a probe appended to `document.body` is already at scale 1 and reports true px — dividing it there inflates the value ~1.875×. See `frame-building`.)
...
authored pass — see `motion`.)

### Export-ready fixed stage
(If you're starting from nothing, `frame-building` emits this whole setup ready to go.)
...
**Multiple independent oscillators?** If the loop layers several non-commensurate motions (per-item float periods + a rotation + a word ticker), one master timeline won't align them — pick a loop length `L` and snap every oscillator's period to an exact integer divisor of `L`, then prove `value(0)==value(L)` AND `velocity(0)==velocity(L)` for *each* one (position AND rotation) plus that every discrete cycler returns to its `t=0` state — before telling the user it's seamless. The frame-based closed form for this lives in `export-as-gif`.
...
always-on `motion` scrollers), `getChildren()[0]` may not be the master; select it
