...
skill (`frame-building`, `motion`, `motion`, `export-as-gif`)
slots in cleanly.
...
<body>
  <div class="stage-scaler">
    <div class="stage">
      <!-- placement goes here (see frame-building) -->
    </div>
  </div>
  ...
  <script>
    ...
    window.addEventListener('DOMContentLoaded', () => {
      ...
      /* Timing tunables live here as named constants (motion fills these in). */

      const build = () => {
        // Build the GSAP timeline here once layout is settled.
        // → motion (reveal loop) / motion (scrolling rows).
      };
...
transform-origin — see `motion`'s emerge-from-point).

**Only divide by the scaler factor for elements INSIDE `.stage-scaler`.** The scaler carries the `transform: scale()`, so only its descendants are scaled. A probe/ruler appended to `document.body` sits *outside* it at scale 1 — its `getBoundingClientRect()` width is already true stage px, and dividing it by the factor inflates it by `1/scale` (~1.875× at a 720px-tall viewport). `offsetWidth`/`offsetLeft` are scale-invariant (layout px) and never need the correction. Confirm body is unscaled with a known-size ruler div (a 100px box on body must measure 100px). This exact slip once shipped inflated word widths and decentered a headline; the port-side version is in `export-as-gif`.

**Assert the read is sane, and magnitude-check it.** `scaler.getBoundingClientRect().width` can come back 0/null right after navigate/reload (→ factor 0 → divide-by-zero → `Infinity`). Wait for stable layout, guard `scale > 0`, and gut-check any measured or hardcoded px against expectation (Latin text at 96px ≈ 50px/char, *not* ~95) — a ~1.9× deviation is a scale-inflation bug; catch it before baking the value in.
...
1. **`frame-building`** — place elements inside `.stage` at Figma-exact geometry.
2. **`motion`** — turn the composition into a seamless loop (fill in `build()`).
3. **`motion`** / **`motion`** — add scrolling rows or orchestrated
