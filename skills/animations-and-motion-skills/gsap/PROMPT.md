...
GSAP animation reference. Covers the core tween methods, vars, transforms, easing,
timelines, and performance. This is the detailed reference; the thin `SKILL.md` router
points here. Load-on-demand extras live in `references/` and `scripts/`.
...
- **[scripts/extract-audio-data.py](scripts/extract-audio-data.py)** — Helper for extracting audio data (e.g. for audio-visualizer effects).
