...
> end-screen, so it can't yoyo — see `loop-animator`), don't use the reverse-leave trick.
...
The card must already be built as separate animatable layers (from `figma-1to1-card`):
...
- Pass to `loop-animator` for timing adjustments to the outer loop
- Copy the `CC_*` constants to `figma-1to1-card` notes so the next rebuild knows the beat map
