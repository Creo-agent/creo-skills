...
Figma's child list is z-order/authoring order, and a section is sometimes 2–3 sibling root nodes
(an illustration + a floating icon + a supporting frame) rather than one clean wrapper. Build a
...
**The one behavioral difference from standalone section mode: brief-skip inference is the
default for every section here, not an interactive back-and-forth per section.** Pausing to ask
a full brief for each of N sections would make a full-page build impractical. State each
section's inferred assumptions once, together, in the final delivery reply (Step 6 below) —
never silently, always visible.
...
controls needing a state-model plan), that section's build still halts and asks for real input
before continuing — brief-skip inference only covers the *inferable* fields (purpose from
copy, page slug, interactivity from the section's own layout pattern), never the genuinely
unspecified ones.
