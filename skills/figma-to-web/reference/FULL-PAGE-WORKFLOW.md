...
**SSE truncation fallback — check before calling, act immediately on failure.** If
`get_metadata` on the page frame fails with `EOF while parsing a string at line 1 column ~40329`,
the response exceeded the SSE transport's ~40KB hard cap. **Do not retry, do not try different
depth values** — neither helps (confirmed Sep 2026). Execute this fallback immediately:

1. Call `get_screenshot(fileKey, nodeId)` on the page frame — this returns a URL only and always
   succeeds regardless of frame size.
2. Download and display the screenshot. Visually identify the top-level sections.
3. Post to the user: _"get_metadata hit the Figma MCP's ~40KB SSE limit on this page frame.
   I can see [N] sections in the screenshot (list them). Please select each section frame in
   Figma and share its URL — each individual section will be well under the limit."_
4. Build the manifest from the per-section URLs as they arrive. Do not wait for all URLs
   before starting — begin Turn 1 scouting on the first URL received.

**Permanent fix:** see `GOTCHAS.md` entry "`get_metadata` / `get_design_context` on a large
page-level frame hard-truncates at ~40KB" — local stdio MCP or desktop bridge eliminates the cap.
