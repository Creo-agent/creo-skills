...
22. **Clay-anchored sections: always check the component's mobile CSS, not just desktop.**
    When a section resolves to a Clay component (EXACT or CLOSEST), the Clay `.module.css` file
    contains responsive rules (typically `@media (min-width: 900px)`) that define how the
    component behaves on mobile. **Read those responsive rules before writing any CSS** — they
    are part of the coded component, not a nice-to-have.
    Confirmed real case (CustomerLogoSection, Aug 2026): the desktop layout was a single-row
    `flex-wrap: nowrap` strip. On mobile, Clay uses `flex-wrap: wrap` so logos flow to a second
    row instead of shrinking to illegible dots. The build copied the desktop layout and only
    shrank logo sizes for mobile — resulting in 8 tiny logos in one row at 390px, identical to
    desktop. The Clay component already solved this with `flex-wrap: wrap`.
    **The rule:** when using Clay as a structural anchor, always read the component's mobile
    breakpoint CSS and carry it into the build. Clay’s mobile behavior is an explicit design
    decision, not a default to ignore.

23. **Section vertical padding: always read from Figma, never rely on the page default.**
    The `padding-section-large` class provides a default vertical padding
    (`clamp(4rem,...,6rem)` ≈ 64–96px). But most Figma sections specify their own padding in
    `paddingTop`/`paddingBottom` on the section frame, and it frequently differs from the
    default — many sections have `padding: [0,0,0,0]` or asymmetric values like `[20,64,40,64]`.
    Confirmed real case (IT page, Aug 2026): 5 out of 15 sections had zero vertical padding in
    Figma. The default added ~192px of phantom height to each, making the page 12.9% taller
    than Figma and creating visible spacing mismatches between sections.
    **The rule:** for every section, read `paddingTop`/`paddingBottom` from the Figma node
    metadata and override `.padding-section-large` in the section's scoped CSS. Do this in the
    same pass as other PRE-BUILD measurements, not after QA flags the height delta.

24. **Desktop-only CSS overrides must be wrapped in a media query — every time, no exceptions.**
    Any CSS change intended only for desktop viewports must be inside
    `@media (min-width: 901px)` (or the page's established desktop breakpoint). This sounds
    obvious, but it is the single most frequent source of mobile regressions in real builds —
    confirmed across multiple pages (PMO, Marketing, IT) where desktop layout tweaks (hero flex
    sizing, card padding removal, enlarged media slots) leaked into mobile and broke stacking,
    caused horizontal overflow, or pushed content off-screen.
    - **The pattern:** someone requests a desktop-only change ("make the video bigger," "remove
      right padding on cards"). The fix is written as a plain CSS rule. It works on desktop. It
      also applies on mobile, where it causes a regression nobody checks until a reviewer catches
      it.
    - **The fix:** wrap every desktop-only override in the media query *at write time*, not after
      a mobile regression is reported. Add `overflow-x: hidden` on `body` as a safety net, but
      do not rely on it — it clips the symptom without fixing the cause.
    - **Cross-ref:** `HARD-RULES.md` H20 covers the specificity case (a mobile variant rule that
      still applies at desktop). This item covers the complementary case: a desktop rule that
      leaks into mobile because it was never scoped.
    - **Detection:** after writing any CSS change, check the page at 390px width before
      committing. If `document.documentElement.scrollWidth > document.documentElement.clientWidth`
      at mobile, a desktop rule leaked.

25. **Persist the full Figma source (fileKey + nodeId), not just the nodeId, for every
    section/asset in the trace/manifest at build time.** Confirmed real case (Sep 2026): a later
    fix session needed to re-check a specific asset's Figma source and had the node id from the
    original build log but not the file key needed to actually query the file — forcing a
    stop-and-ask (`AskUserQuestion`) for the Figma URL that a maintenance session shouldn't need
    to re-request. Log the complete `https://figma.com/design/<fileKey>/...?node-id=<id>` (or the
    fileKey/nodeId pair separately) for every section and every individually-sourced asset, not
    just the nodeId alone — a nodeId with no fileKey is not enough to look anything up later.
