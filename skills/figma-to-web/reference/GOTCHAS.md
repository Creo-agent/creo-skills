...
- `get_metadata` / `get_design_context` on a large page-level frame hard-truncates at ~40KB in the SSE transport — depth param has no effect, retrying never helps
...
## `get_metadata` / `get_design_context` on a large page-level frame hard-truncates at ~40KB — depth param has no effect, retrying never helps

Confirmed real case (Sep 2026, Re-Brand 2026 file, node 3808:11746): calling `get_metadata` or
`get_design_context` on a large full-page frame consistently fails with:

```
Failed to parse SSE message: Invalid JSON: EOF while parsing a string at line 1 column 40329
```

The truncation point (40329 / 40253 chars) is a hard SSE transport buffer cap, not a token
limit. **Depth parameter has zero effect** — `depth=1`, `depth=2`, and no-depth all fail
identically at the same column. Retrying the same call never helps. This is not a transient
network issue.

**What still works on the same node:** `get_screenshot` and `download_assets` — both return a
URL (not the full JSON tree), so they stay well under the cap.

**Immediate fallback (no environment change needed):**
1. Do not retry. Do not try different depth values.
2. Take `get_screenshot` on the page frame to visually identify sections.
3. Ask the user to select each top-level section frame in Figma and share its URL.
   Each individual section node will be far smaller than the full-page frame and will
   fit within the 40KB limit — `get_metadata` and `get_design_context` work normally on them.
4. Build the section manifest from the per-section URLs and proceed.

**Permanent computer-level fixes (eliminate the cap entirely):**
- **Local stdio MCP (best):** run `@figma/mcp-server` locally via stdio — no SSE transport,
  no size cap. Add to `~/.claude/settings.json` under `mcpServers`:
  ```json
  "figma-local": {
    "command": "npx",
    "args": ["-y", "@figma/mcp-server"],
    "env": { "FIGMA_API_KEY": "YOUR_FIGMA_PAT_HERE" }
  }
  ```
  Replace `YOUR_FIGMA_PAT_HERE` with a Figma Personal Access Token from
  figma.com → Settings → Security → Personal Access Tokens.
- **Desktop bridge:** authorize `plugin:figma:figma` via `claude mcp auth plugin:figma:figma`
  in an interactive terminal session, then restart Claude Code.
- **MAX_MCP_OUTPUT_TOKENS:** add `"MAX_MCP_OUTPUT_TOKENS": "100000"` to the `env` block in
  `~/.claude/settings.json`. Fixes the token-limit variant of this error; may not fix the SSE
  JSON-truncation variant (different failure mode, same symptom).
