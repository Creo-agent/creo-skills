...
**Values follow H22**, not the original page-structure skill's hardcoded `80rem` / `2.5rem`.
On the first section of a page, establish one pair:
...
3. Fall back only if neither exists:
...
.container-large {
  width: 100%;
  max-width: var(--clay-layout-container-large, 80rem);
  margin: 0 auto;
}
.padding-section-large {
